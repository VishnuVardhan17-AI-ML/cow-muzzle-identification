import cv2
import os
import pandas as pd

input_folder = "dataset/production_test"

original_folder = "outputs/original"
clahe_folder = "outputs/clahe"

os.makedirs(original_folder, exist_ok=True)
os.makedirs(clahe_folder, exist_ok=True)

clahe = cv2.createCLAHE(
    clipLimit=2.0,
    tileGridSize=(8, 8)
)

results = []

for file in os.listdir(input_folder):

    path = os.path.join(
        input_folder,
        file
    )

    img = cv2.imread(path)

    if img is None:
        continue

    gray = cv2.cvtColor(
        img,
        cv2.COLOR_BGR2GRAY
    )

    enhanced = clahe.apply(gray)

    cv2.imwrite(
        os.path.join(original_folder, file),
        gray
    )

    cv2.imwrite(
        os.path.join(clahe_folder, file),
        enhanced
    )

    blur_score = cv2.Laplacian(
        gray,
        cv2.CV_64F
    ).var()

    contrast_before = gray.std()
    contrast_after = enhanced.std()

    brightness_before = gray.mean()
    brightness_after = enhanced.mean()

    contrast_change = (
        contrast_after -
        contrast_before
    )

    brightness_change = (
        brightness_after -
        brightness_before
    )

    if contrast_change > 0:
        status = "Improved"
    else:
        status = "Degraded"

    animal_id = "_".join(
        file.split("_")[:-1]
    )

    results.append([
        file,
        animal_id,
        round(blur_score, 2),
        round(contrast_before, 2),
        round(contrast_after, 2),
        round(brightness_before, 2),
        round(brightness_after, 2),
        round(contrast_change, 2),
        round(brightness_change, 2),
        status
    ])

df = pd.DataFrame(
    results,
    columns=[
        "image_name",
        "animal_id",
        "blur_score",
        "contrast_before",
        "contrast_after",
        "brightness_before",
        "brightness_after",
        "contrast_change",
        "brightness_change",
        "status"
    ]
)

df.to_csv(
    "comparison_report.csv",
    index=False
)

print(df.head())
print("\nSaved comparison_report.csv")